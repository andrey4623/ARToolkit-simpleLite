...
static void mainLoop(void)
{
	...
	if ((image = arVideoGetImage()) != NULL) {
		...
		if (gARTImageSavePlease) {
			...
			sprintf_s(imageNumberText, "image-%04d.jpg", imageNumber++);
...
int main(int argc, char** argv)
{
	...
	if (!windowed) {
		if (windowRefresh) sprintf_s(glutGamemode, "%ix%i:%i@%i", windowWidth, windowHeight, windowDepth, windowRefresh);
		else sprintf_s(glutGamemode, "%ix%i:%i", windowWidth, windowHeight, windowDepth);
...
static void printMode()
{
	...
	_snprintf_s(text, sizeof(text), "Processing %dx%d video frames %s", xsize, ysize, text_p);
	...
	_snprintf_s(text, sizeof(text), "Threshold mode: %s", text_p);
